...
Weak pattern detection using Regex, including:
Sequences (123, abc)
Repetitions (aaa, 111)
...
Length
Character variety
Heuristic penalties using Regex pattern detection

### Example:
/^\d+$/           // only numbers
/(.)\1{2,}/       // repeated characters
/^(.{1,3})\1+$/   // repeated patterns
The final score is normalized to ensure consistent visual representation in the strength bar.